...
func _ready() -> void:
	...
	var player_velocity = Vector2.ONE*10
	var player_postion =Vector2(center.x,center.y) - Vector2(4*radius_of_moon,4*radius_of_moon) 
	
	$Ship_Scene/CharacterBody2D.rotate(3*PI/4)
	...
	instr.get_node("Label").text = "You enter a local system from a distant system. Enter orbit around the nearby planet. Make sure to say withing the marked region for 20 seconds \n \n PRESS ENTER TO START \n \n  Hint: start by getting the peak of you arc into the region and then circulizing your orbit from the peak."
	if !instr.get_is_on():
		instr.turn_on()
